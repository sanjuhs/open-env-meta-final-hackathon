...
app.innerHTML = `
  <main class="shell">
    <section class="panel controls">
      <div>
        <p class="eyebrow">MechForge Experiment</p>
        <h1>Prompt to simulated mechanical design</h1>
        <p class="intro">Ask GPT or the tool episode to construct a part through visible CAD operations, then run the Python verifier on the resulting state.</p>
      </div>

      <label for="prompt">Design prompt</label>
      <div class="preset-row">
        <button data-preset="cantilever">Cantilever</button>
      ...
      <textarea id="prompt" spellcheck="false">Design a lightweight 6061 aluminum cantilever bracket. It is fixed by two M5 bolts on the left side and must carry 120 N downward at the tip 90 mm from the fixed edge. Keep mass below 45 g and safety factor above 2.0.</textarea>
    ...
    <section class="viewer-wrap">
      <div class="viewer-top">
        <div>
          ...
          <h2 id="design-title">No design loaded</h2>
        </div>
        <div class="legend">
          <span><i class="swatch plate"></i>base/body</span>
...
const promptPresets = {
  ...
  chair:
    "Design a simple full chair for a 700 N downward seated load. It should visibly look like a chair with a seat panel, four legs, crossbars, and a backrest, not a flat bracket or just a support frame.",
  "advanced-chair":
    "Design a chair which is very curvy. It has flat feet and armrest handles as well. It is relatively ergonomic, with a curved seat and a curved back. It also has another headrest. It is an advanced chair and beautiful looking. It can withstand a force of 1000 N as people sit on it and another 100 N on the backrest.",
  table:
...
function renderAgentLoop(run) {
  if (!run) {
    const empty = `<p class="empty-note">Run Generate, Run 5.4 Iterations, or Run Tool Episode to inspect the loop.</p>`;
  ...
  if (run.type === "tool_episode") {
    ...
    agentLoopEl.innerHTML = `
      <div class="loop-strip">
        <span>Prompt</span><span>Select family</span><span>CAD operations</span><span>CadQuery export</span><span>Python 3D FEA</span><span>Commit</span>
      </div>
      <h3>Tool Episode</h3>
  ...
  agentLoopEl.innerHTML = `
    <div class="loop-strip">
      <span>Prompt</span><span>LLM design JSON</span><span>Python 3D FEA</span><span>Feedback summary</span><span>Next iteration</span>
    </div>
    <h3>Iterations</h3>
  `;
  iterationsEl.innerHTML = `
...
function exportStl() {
  ...
  link.href = url;
  link.download = "mechforge-design.stl";
  link.click();
